 ...
 * 事件监听模式使用在客户端RIA比较多，因为这里是用户输入的源头，是事件发生的源头。而且在目前WOA（Web-oriented architecture）趋势下，事件监听模式不能只单独局限于RIA客户端这个范围，还需要把事件通过http形式传递到服务器端，也就是跨客户端和服务器的，事件必须和服务器的PUSH异步结合在一起。这是一种先进的架构设计。
 * 订阅发布者模式有时也称为观察者模式。
 *
 * 以下是同步模式:
 *
...
public class Client {
    ...
    public Client() {
        try {
            ...
            es.addListener(new MyEventListener() {
                @Override
                public void handleEvent(MyEventObject event) {
                    System.out.println("Method come from anonymous...");
                }
            });
            // 触发事件、通知监听器
            es.notifyEventObject();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
